...
def dbinPYthon():
    ...
    return link.votes
print "SELECT * FROM links WHERE id = 2:\t", dbinPYthon()
...
def advSQLinPYthon():
    ...
    link = Link(*c.fetchone())
    return link.id
print "SELECT * FROM links WHERE submitter_id = 62443 AND votes > 1000:\t", advSQLinPYthon()
...
def OrderBy():
    ...
    for link_tuple in c:
        link = Link(*link_tuple)
        newlist.append(link.id)
    return newlist
print "SELECT * FROM links WHERE submitter_id = 62443 ORDER BY submitted_time ASC:\t", OrderBy()


# QUIZ - implement the function link_by_id() that takes a link's ID and returns
# the Link object itself
def link_by_id(link_id): # QUIZ Querying Links
    for l in links:
        if l.id == link_id:
            return l
print "Link 24:\t", link_by_id(24)

# QUIZ - implement the function build_link_index() that creates a python dictionary
# the maps a link's ID to the link itself
def build_link_index(): # QUIZ Using Dictionaries as Indexes
    index = {}
    for l in links:
        index[l.id] = l
    return index

link_index = build_link_index() # BAD Global Variable, for this example it's fine

def link_by_id2(link_id):       # BETTER link by id function using a hash table
    return link_index.get(link_id)

print link_by_id2(24), '\n'      # Returns Link 24
print link_by_id2(50) # Returns NONE (that's what the get does, rather than using link_index[link_id]


# QUIZ - implement the function add_new_link() that both adds a link to the
# "links" list and updates the link_index dictionary.
def add_new_link(link): # QUIZ Lookup
    links.append(link)              # Append the link to the links list
    link_index[link.id] = link      # Add the key-value to the index

add_new_link(Link(50, 1, 1, 1,
         "TITLE",
         "URL"))
